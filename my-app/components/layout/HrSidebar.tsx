...
const HrSidebar = () => {
  ...
  return (
    <>
      <aside className="border-r border-zinc-600 bg-[#0F0E23] w-56 md:w-60 lg:w-70 xl:w-80 2xl:w-96 p-3 md:p-5 sticky top-0 min-h-screen h-screen flex flex-col shrink-0">
        ...
        <nav className="flex-1 min-h-0 overflow-y-auto py-4">
          <ul className="text-center ps-3 md:ps-5 xl:ps-10 space-y-2 md:space-y-4">
            {menu.map((item, id) => {
              const Icon = item.icon;
              return (
                <Link href={item.href} key={id}>
                  <li className="w-max flex items-center gap-2 md:gap-3 text-sm md:text-base lg:text-lg xl:text-xl text-zinc-400 hover:text-white hover:scale-105 transition-all duration-300">
